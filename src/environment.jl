...
function defaultEnv()
    ...
    define(env, symbol("+"), procedure(args -> Num(args.car.value + args.cdr.car.value)))
    define(env, symbol("-"), procedure(args -> Num(args.car.value - args.cdr.car.value)))
    define(env, symbol("*"), procedure(args -> Num(args.car.value * args.cdr.car.value)))
    define(env, symbol("/"), procedure(args -> Num(args.car.value / args.cdr.car.value)))
    define(env, symbol("^"), procedure(args -> Num(args.car.value ^ args.cdr.car.value)))
    define(env, symbol("today!"), procedure(args -> today()))
    define(env, symbol("list"), procedure(args -> args))
    define(env, QUOTE, special((s, args, env) -> args))
